...
export function InstalacionCard({
  ...
  return (
    <Card className="hover:shadow-lg transition-all duration-200 flex flex-col justify-between border-slate-200 overflow-hidden bg-white">
      {/* Encabezado */}
      <CardHeader className="pb-3 border-b border-slate-100 bg-slate-50/40">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="p-2 bg-red-50 text-brand-red rounded-xl border border-red-100 shadow-2xs shrink-0">
              <Dumbbell className="h-4 w-4" />
            </div>
            <div className="min-w-0">
              <CardTitle className="text-sm font-bold text-slate-900 leading-tight truncate">
                {instalacion.nombre}
              </CardTitle>
      ...
      <CardContent className="space-y-4 pt-3.5 pb-4 flex-1">
        {/* Descripción */}
        ...
        <div className="bg-slate-50/80 rounded-xl p-3 border border-slate-200/80 space-y-2.5">
            ...
            {DIAS_SEMANA_CONFIG.map(({ key, shortLabel }) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'flex flex-col items-center justify-center py-1 px-0.5 rounded-md text-[9px] sm:text-[10px] font-bold transition-all text-center min-w-0',
                    isOpen
          ...
          {/* Lista detallada de días que abre y sus rangos horarios */}
          {horariosList.length > 0 ? (
            <div className="space-y-1.5 pt-1 max-h-48 overflow-y-auto pr-1">
              {horariosList.map((d) => (
                <div
                  key={d.diaSemana}
                  className="flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-white border border-slate-200/70 text-xs shadow-2xs hover:border-blue-200 transition-colors gap-2"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-xs shrink-0" />
                    <span className="font-bold text-slate-800 text-[11px] truncate">{d.label}</span>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span className="font-mono text-[11px] font-semibold text-slate-700 bg-slate-50 px-2 py-0.5 rounded border border-slate-200 whitespace-nowrap">
            ...
            <div className="flex items-center gap-2 p-3 bg-white rounded-lg border border-dashed border-slate-200 text-slate-500 text-xs">
              <AlertCircle className="h-4 w-4 text-amber-500 shrink-0" />
              <span>Sin horarios configurados (Cerrada para reservas)</span>
            </div>
          )}
        </div>
      ...
      <CardFooter className="bg-slate-50/60 p-4 border-t border-slate-100 flex flex-col gap-3">
        {/* Selector de Estado */}
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className={cn(
                'h-7 px-2 border rounded-md text-[10px] font-bold cursor-pointer focus:outline-none transition-colors',
        ...
        <div className="grid grid-cols-2 gap-2 w-full">
          <Button
            ...
            className="w-full h-8 text-[11px] font-bold flex items-center justify-center gap-1.5 cursor-pointer border-blue-200 text-blue-700 hover:bg-blue-50 transition-colors"
          >
          ...
          <Button
            ...
            className="w-full h-8 text-[11px] font-bold flex items-center justify-center gap-1.5 cursor-pointer hover:bg-slate-100 transition-colors"
          >
            <Edit className="h-3.5 w-3.5 shrink-0" />
            <span className="truncate">Editar Info</span>
          </Button>
        </div>
      </CardFooter>
    </Card>
